...
export default function DoctorChatRoomPage({
  ...
  React.useEffect(() => {
    if (!lastJsonMessage) return
    if (lastJsonMessage.sender_id === data?.data.doctor_id) return
    if (lastJsonMessage.message || lastJsonMessage.attachment) return
      ...
      <section className="col-span-8 flex size-full flex-col justify-stretch border-x">
        <header className="relative flex h-full max-h-[80px] items-center gap-4 border-b p-3">
          ...
          <Avatar className="size-12 rounded-full bg-muted">
            <AvatarImage
              src={data?.data.user.profile_photo ?? undefined}
                  ...
                  {data && (
                    <ul className="space-y-4 p-4" ref={listEl}>
                      {messageHistory.map((message, idx) => {
                        const isUser = message.sender_id === data.data.doctor_id
                        return (
                          <li
                            key={idx}
                            className={cn(
...
DoctorChatRoomPage.getLayout = function getLayout(page: React.ReactElement) {
